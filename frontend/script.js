...
const placeholders = [
    'Ask about pickleball rules, strategy, or technique...',
    'What is the kitchen rule?',
    'How do I hit a third shot drop?',
    'What is dinking and when should I use it?',
    'How does scoring work in pickleball?',
    'What are common mistakes beginners make?',
    'How do I improve my serve?',
    'What is poaching in doubles?'
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Rotate placeholder every 3s when input is empty and unfocused
    setInterval(() => {
...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (!isWelcome) {
        const copyBtn = type === 'assistant'
            ? `<button class="copy-btn" onclick="copyMessage(this, ${messageId})" title="Copy response">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                Copy
              </button>`
            : '';
        html += `<div class="message-footer"><span class="message-time">${time}</span>${copyBtn}</div>`;
    }
...
function copyMessage(btn, messageId) {
